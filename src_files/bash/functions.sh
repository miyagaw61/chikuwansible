...
rep() {
    if [ "$1" = "-h" ] ;then
        ...
        if [ $# -gt 1 ] ;then
            ...
            cat $tmp_file | sed -E "s@(^ *)([0123456789]+)(.*)@\1\x1b[0;32m\2\x1b[0m\3@g" | sed -E "s@($regexp)@\x1b[1;4;31;44m\1\x1b[0m@g"
        else
            rg "$regexp" -n --line-number-width=7 | sed -E "s@(^ *)([0123456789]+)(.*)@\1\x1b[0;32m\2\x1b[0m\3@g" | sed -E "s@($regexp)@\x1b[1;4;31;44m\1\x1b[0m@g"
        fi
        rm -rf $tmp_file
    fi
}
